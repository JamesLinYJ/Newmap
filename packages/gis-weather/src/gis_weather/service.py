...
def is_supported_weather_file(filename: str) -> bool:
    return Path(filename).suffix.lower() in SUPPORTED_WEATHER_SUFFIXES
...
class WeatherDataService:
    def inspect(self, path: Path, *, filename: str | None = None) -> dict[str, Any]:
        ...
        if suffix not in SUPPORTED_WEATHER_SUFFIXES:
        ...
        *,
        ...
        for variable in variables[: max(0, int(stats_variable_limit))]:
            ...
            if not name or not variable.get("analysisReady", True):
                continue
            try:
                stats_rows.append(self.stats(path, variable=name))
            except Exception as exc:
                stats_rows.append({"variable": name, "error": str(exc).strip() or exc.__class__.__name__})
        return write_weather_report_docx(
    ...
    def _read_map_grid(
        ...
        *,
        ...
        return self.read_grid(path, variable=variable, time_index=time_index, level_index=level_index)
    ...
    def _read_raster_grid(self, path: Path, *, variable: str | None) -> WeatherGrid:
        rasterio = _rasterio()
        with rasterio.open(path) as src:
    ...
    def _read_hdf5_grid(self, path: Path, *, variable: str | None, xarray_error: Exception | None = None) -> WeatherGrid:
        h5py = _h5py()
    ...
    def _read_radar_grid(self, path: Path, *, variable: str | None, elevation_index: int | None) -> WeatherGrid:
        decoded = decode_radar_bz2(path)
...
def _require_1d_lat_lon(grid: WeatherGrid) -> tuple[Any, Any]:
