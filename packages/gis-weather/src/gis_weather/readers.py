...
class WeatherDatasetReader(Protocol):
    def supports(self, path: Path) -> bool:
        ...

    def inspect(self, path: Path, *, filename: str | None = None) -> WeatherDatasetIndex:
        ...

    def read_slice(self, path: Path, query: GridQuery) -> GridSlice:
        ...


class XarrayScientificReader:
    def supports(self, path: Path) -> bool:
        return path.suffix.lower() in {".nc", ".nc4", ".grib", ".grb", ".grb2", ".h5", ".hdf5"}

    def inspect(self, path: Path, *, filename: str | None = None) -> WeatherDatasetIndex:
        xr = _xarray()
        ds, engine = _open_xarray_dataset(xr, path)
        try:
            ...
            return WeatherDatasetIndex(
                filename=filename or path.name,
                format=_format_label(path.suffix.lower()),
                engine=engine,
                variables=variables,
    ...
    def read_slice(self, path: Path, query: GridQuery) -> GridSlice:
        xr = _xarray()
        ds, _engine = _open_xarray_dataset(xr, path)
        try:
            lat_name, lon_name = _find_lat_lon_coords(ds)
...
class WeatherReaderFacade:
    # Reader 路由器。
    #
    # 上层只构造 GridQuery；具体由哪个 backend 执行在这里决定。
    def __init__(self, readers: list[WeatherDatasetReader] | None = None):
        self.readers = readers or [RasterMapReader(), XarrayScientificReader()]

    def inspect(self, path: Path, *, filename: str | None = None) -> WeatherDatasetIndex:
        return self._reader_for(path).inspect(path, filename=filename)

    def read_slice(self, path: Path, query: GridQuery) -> GridSlice:
        return self._reader_for(path).read_slice(path, query)

    def _reader_for(self, path: Path) -> WeatherDatasetReader:
        for reader in self.readers:
            if reader.supports(path):
                return reader
        raise ValueError(f"不支持的气象文件格式：{path.suffix or 'unknown'}")
...
def _open_xarray_dataset(xr: Any, path: Path) -> tuple[Any, str]:
