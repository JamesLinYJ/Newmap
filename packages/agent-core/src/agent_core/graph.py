        ...
        *,
        ...
        if plan_mode_enabled:
            lines.extend(
                [
                    ...
                    "- 进入探索前先调用 **todo_write** 写出计划模式待办；每完成一个探索步骤都更新待办状态。",
                ]
            )
        ...
        lines.extend(
            [
                ...
                "- 除非一次回答即可完成，否则先调用 **todo_write** 写出 2-6 个待办项；每完成一步立刻调用 **todo_write** 更新 pending/running/completed 状态。自动模式也必须善用 Todo，不要只在文本里描述计划。",
