...
// =========================================================================
// Better Auth core tables — 认证与会话事实源
// =========================================================================
export const authUser = pgTable(
  ...
  (table) => [uniqueIndex('idx_auth_user_email_unique').on(table.email)],
)
...
export const authSession = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_auth_session_token_unique').on(table.token),
...
export const authAccount = pgTable(
  ...
  (table) => [index('idx_auth_account_user_id').on(table.userId)],
)
...
// =========================================================================
// platform_users / workspaces / memberships — 身份与工作区成员关系
// =========================================================================
export const platformUsers = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_platform_users_subject_unique').on(table.subject),
    uniqueIndex('idx_platform_users_email_unique').on(table.email),
  ],
)
...
export const platformMemberships = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_platform_memberships_member_role_unique').on(table.workspaceId, table.userId, table.role),
    index('idx_platform_memberships_workspace').on(table.workspaceId),
    index('idx_platform_memberships_user').on(table.userId),
  ],
)

export const platformRbacPolicies = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_platform_rbac_policy_unique').on(table.ptype, table.v0, table.v1, table.v2, table.v3, table.v4, table.v5),
  ],
)
