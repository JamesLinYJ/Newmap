...
这个目录保存从 `C:\Users\James\Desktop\source-code` 复制过来的工具源码快照，作为后续迁移 Newmap 计划模式、文件工具、搜索工具、任务工具和 Agent 工具的参考来源。

## 边界

- `src/tools/`、`src/Tool.ts`、`src/tools.ts` 是原始工具接口和工具实现快照。
- 这里的文件不参与 Newmap `server` 或 `apps/web` 编译。
- 后续运行态必须通过 Newmap 自己的 `ToolProvider`、`ToolRegistry`、Agents SDK bridge、approval、artifact 和 valueRef 链路适配。
