...
async function handleResponse<T>(response: Response): Promise<T> {
  // fetch 裸请求响应处理。
  //
  // 少数接口需要 AbortSignal.timeout 等浏览器 API，不能直接复用 requestJson；
  // 但错误提取仍走同一套规则，避免 UI 得到不一致的失败文案。
  if (!response.ok) {
    throw new Error(await extractErrorDetail(response))
  }
  return (await response.json()) as T
...
export function startAnalysis(sessionId: string, query: string, provider?: string, model?: string, clarificationOptionId?: string | null, executionMode: AgentExecutionMode = 'auto') {
  ...
  return requestJson<AnalysisRun>('/api/v2/runs', {
    method: 'POST',
    body: JSON.stringify({ sessionId, query, modelProvider: provider, modelName: model, clarificationOptionId, executionMode }),
  }, 60_000)
}
...
export function startThreadRun(threadId: string, query: string, provider?: string, model?: string, clarificationOptionId?: string | null, executionMode: AgentExecutionMode = 'auto') {
  ...
  return requestJson<AnalysisRun>(`/api/v2/threads/${threadId}/runs`, {
    method: 'POST',
    body: JSON.stringify({ query, modelProvider: provider, modelName: model, clarificationOptionId, executionMode }),
  })
}
...
export function getArtifacts(runId: string) {
  return requestJson<ArtifactRef[]>(`/api/v2/runs/${runId}/artifacts`)
}
...
export function openRunEventStream(
  ...
  const source = new EventSource(`${API_BASE_URL}/api/v2/runs/${runId}/events`)
  const handleEvent = (message: MessageEvent<string>) => {
    try {
      onEvent(JSON.parse(message.data) as RunEvent)
    } catch {
      // 后端返回畸形 JSON 时丢弃本条消息，不中断诊断事件流。
    }
  }
...
export function openRunItemStream(
  runId: string,
  ...
  // Agent 对话 item 流。
  //
  // 聊天 UI 的实时展示只消费 ConversationItem；RunEvent 只进入调试视图。
  const source = new EventSource(`${API_BASE_URL}/api/v2/runs/${runId}/items/stream`)
  const handleItem = (message: MessageEvent<string>) => {
    onItem(JSON.parse(message.data) as ConversationItem)
  }
  source.addEventListener('item/started', handleItem)
  source.addEventListener('item/delta', handleItem)
  source.addEventListener('item/completed', handleItem)
  source.onerror = onError
  return source
}
