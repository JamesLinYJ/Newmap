...
#   地理智能平台 - 气象科学链路测试
#
#   文件:       test_weather_scientific_chain.py
#
#   日期:       2026年06月08日
#   作者:       JamesLinYJ
# --------------------------------------------------------------------------
...
from gis_weather import NowcastTextService
from gis_weather.nowcast import build_analysis_scope
from gis_weather.third_party.radar_mosaic_agent import adapter as radar_adapter
...
def test_worker_reads_extensionless_runtime_object_with_original_filename(
    ...
    metadata = sidecar.execute_weather_tool("inspect_meteorological_dataset", common)
    stats = sidecar.execute_weather_tool("meteorological_stats", {**common, "variable": "QPF"})
    raster = sidecar.execute_weather_tool(
...
def test_generated_netcdf_nowcast_reference_chain(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    sequence = sidecar.execute_weather_tool("create_nowcast_sequence", {"files": files, "variable": "rain"})
    inspection = sidecar.execute_weather_tool("inspect_nowcast_sequence", {"sequence": sequence})
    analysis = sidecar.execute_weather_tool("analyze_nowcast_precipitation", {"sequence": sequence})
    answer = sidecar.execute_weather_tool("answer_nowcast_question", {"analysis": analysis, "question": "未来会下雨吗？"})
    forecast = sidecar.execute_weather_tool("generate_nowcast_forecast_text", {"analysis": analysis})
...
def test_third_party_risk_map_and_area_rainfall_outputs_are_real_files(tmp_path: Path) -> None:
    # 风险区划图和面雨量表格走真实 xarray/geopandas/matplotlib/openpyxl 链路。
