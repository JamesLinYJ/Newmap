...
describe('Hangzhou nowcast tools', () => {
  it('keeps the weather manifest and runtime definitions identical', () => {
    expect(() => validateToolProvider(provider)).not.toThrow()
  })

  it('does not hide weather tools when the worker is temporarily unreachable during provider load', async () => {
  ...
  it('creates a district boundary valueRef from an existing boundary reference', async () => {
    ...
    const tool = weatherTools.find(candidate => candidate.name === 'prepare_hangzhou_nowcast_scope')!
  ...
  it('delivers the standard answer together with a peak-time raster artifact', async () => {
    ...
    const tool = weatherTools.find(candidate => candidate.name === 'answer_nowcast_question')!
  ...
  it('passes the full radar mosaic contract to the worker and records provenance', async () => {
    ...
    const tool = weatherTools.find(candidate => candidate.name === 'render_radar_mosaic')!
    const result = await tool.handler({
    ...
    expect(result.provenance).toMatchObject({
      ...
      sourceSnapshot: 'packages/gis-weather/src/gis_weather/third_party/radar_mosaic_agent/source/original',
  ...
  it('inspects radar station collections and emits target-time refs', async () => {
    ...
    const tool = weatherTools.find(candidate => candidate.name === 'inspect_radar_station_collection')!
  ...
  it('compares radar mosaic output with a NetCDF reference and records slider artifacts', async () => {
    ...
    const tool = weatherTools.find(candidate => candidate.name === 'compare_radar_mosaic_reference')!
    const result = await tool.handler({
  ...
  it('hard-fails third-party tools when the valueRef kind is wrong', async () => {
    ...
    const tool = weatherTools.find(candidate => candidate.name === 'render_rainfall_risk_map')!
  ...
  it('sends the original filename with dataset object paths', async () => {
    ...
    const tool = weatherTools.find(candidate => candidate.name === 'inspect_meteorological_dataset')!
  ...
  it('creates rainfall thresholds and area rainfall artifacts with third-party provenance', async () => {
    stubRuntimeEnv()
    const thresholdTool = weatherTools.find(candidate => candidate.name === 'define_rainfall_risk_thresholds')!
    ...
    expect(thresholds.provenance).toMatchObject({
      ...
      sourceSnapshot: 'packages/gis-weather/src/gis_weather/third_party/rainfall_risk_map/source/original',
    })
    ...
    const tableTool = weatherTools.find(candidate => candidate.name === 'generate_area_rainfall_table')!
    ...
    expect(table.provenance).toMatchObject({
      ...
      sourceSnapshot: 'packages/gis-weather/src/gis_weather/third_party/short_term_forecast/source/original',
    })
  })
})
...
function stubRuntimeEnv() {
  ...
  vi.stubEnv('ENABLED_TOOL_PROVIDERS', 'weather')
}
