...
- 降雨风险区划图、面雨量表格和杭州短临区划分析都必须使用已有边界引用；后续工具参数传 valueRef ID，不复制 GeoJSON。
...
- 降雨风险区划图不使用 create_nowcast_sequence 的 nowcast_sequence 作为 dataset_ref。除非用户明确要求短临问答、连续时次趋势或面雨量排行，否则不要为风险区划图创建 nowcast_sequence。
